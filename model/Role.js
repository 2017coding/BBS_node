...
class Role extends Base{
  ...
  async getAll (obj) {
    // 查询到当前角色和当前角色的所有下级角色
    let sql, createRoleList = await this.getCreateRole(obj.get.pid || obj.get.id)
    try {
      sql = `select a.*, b.name as create_user_name, c.name as update_user_name, d.name as pName from bbs_role as a
          ...
          where 1 = 1 and a.id in (${mysql.escape([obj.get.pid || obj.get.id, ...createRoleList])}) and a.flag = 1`
    } catch (e) {
      return e
    }
    return query(sql)
