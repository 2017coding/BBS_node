...
class Role extends Base{
  ...
  async create (obj) {
    const sql = `INSERT INTO bbs_role set ${this.joinStr('set', obj.set)}`
    return query(sql)
  }
  async update (obj) {
    const sql = `UPDATE bbs_role set ${this.joinStr('set', obj.set)} where 1 = 1 ${this.joinStr('get', obj.get)}`
    return query(sql)
  }
  async delete (obj) {
    const sql = `DELETE from bbs_role where 1 = 1 ${this.joinStr('get', obj.get)}`
    return query(sql)
  }
  async getRow (obj) {
